...
class ModelEvaluator(object):
    ...
    def evaluate_model(self, data):
        try:
            ...
            participants_results_grouped["final_prediction"] = (
                participants_results_grouped["predicted_prob"] >= 0.5
            ).astype(int)

            # Calculate the metrics
            accuracy_A = accuracy_score(
                participants_results_grouped["true_label"],
                participants_results_grouped["final_prediction"],
            )
            balanced_accuracy_BA = balanced_accuracy_score(
                participants_results_grouped["true_label"],
                participants_results_grouped["final_prediction"],
            )
            ...
            accuracy_female = accuracy_score(
                female_data["true_label"], female_data["final_prediction"]
            )
            balanced_accuracy_female = balanced_accuracy_score(
                female_data["true_label"], female_data["final_prediction"]
            )
            accuracy_male = accuracy_score(male_data["true_label"], male_data["final_prediction"])
            balanced_accuracy_male = balanced_accuracy_score(
                male_data["true_label"], male_data["final_prediction"]
            )

            # Calculate Equality of Opportunity (EO)
            conf_matrix_female = confusion_matrix(
                female_data["true_label"], female_data["final_prediction"]
            )
            conf_matrix_male = confusion_matrix(
                male_data["true_label"], male_data["final_prediction"]
            )
            tpr_female = conf_matrix_female[1, 1] / (
                conf_matrix_female[1, :].sum() if conf_matrix_female[1, :].sum() != 0 else 1
            )
            tpr_male = conf_matrix_male[1, 1] / (
                conf_matrix_male[1, :].sum() if conf_matrix_male[1, :].sum() != 0 else 1
            )
